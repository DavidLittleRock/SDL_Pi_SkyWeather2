...
def calc_wind_quadrant(wind_dir):
    quadrants = ["N", "NNE", "NE", "ENE", "E", "ESE", "SE", "SSE", "S", "SSW",
...
def generateCurrentWeatherJSON():
    try:
        ...
        if (len(rainspanrecords) > 0):
            rainspan = rainspanrecords[len(rainspanrecords) - 1][1] - \
                       rainspanrecords[0][1]
        else:
            rainspan = 0
...
def fetchWindData(timeDelta):
    try:
        ...
        for single in df:
            for i in range(0, 8):
                single[i] = round(
                    100.0 * float(single[i]) / float(totalRecords), 2)
...
df = [77.5, 72.5, 70.0, 45.0, 22.5, 42.5, 40.0, 62.5]
    ...
    fig.add_trace(go.Barpolar(
            r=df[4],
            name=returnNumberConverted(8.5) + '-' + returnNumberConverted(
                11) + " " + WUnits(),
    ...
    fig.add_trace(go.Barpolar(
            r=df[3],
            name=returnNumberConverted(4.4) + '-' + returnNumberConverted(
                8.5) + " " + WUnits(),
            marker_color='rgb(120,0,163)'
    ))
    fig.add_trace(go.Barpolar(
            r=df[2],
            name=returnNumberConverted(2.2) + '-' + returnNumberConverted(
                4.4) + " " + WUnits(),
            marker_color='rgb(160,0,163)'
    ))
    fig.add_trace(go.Barpolar(
            r=df[1],
            name=returnNumberConverted(1.0) + '-' + returnNumberConverted(
                2.3) + " " + WUnits(),
...
def buildOutdoorTemperature_Humidity_Graph_Figure():
    ...
    fig.update_yaxes(title_text="<b>Humidity (%)</b>", range=(0, 100),
...
def fetchAQI(timeDelta):
    if (config.USEWSAQI):
        try:
            ...
            before = now - timeDelta
            before = before.strftime('%Y-%m-%d %H:%M:%S')
            query = "SELECT AQI, AQI24Hour, timestamp FROM `AQI433MHZ` WHERE (timestamp > '%s') ORDER BY id ASC" % (
                before)
            # print("query=", query)
            cur.execute(query)
            con.commit()
        ...
        try:
            ...
            before = now - timeDelta
            before = before.strftime('%Y-%m-%d %H:%M:%S')
            query = "SELECT AQI, AQI24Average, TimeStamp FROM `WeatherData` WHERE (TimeStamp > '%s') ORDER BY id ASC" % (
                before)
            # print("query=", query)
            cur.execute(query)
            con.commit()
...
def buildAQIGraphFigure():
    ...
    # Set y-axes titles
    fig.update_yaxes(title_text="<b>AQI 24H Ave</b>", range=(minTemp, maxTemp),
                     secondary_y=False, side='left')
    fig.update_yaxes(title_text="<b>AQI </b>", range=(minTemp, maxTemp),
                     secondary_y=True, side='right')
    ...
    Row1 = html.Div(
            [
                ...
                dbc.Row(dbc.Col(html.Div(
                    html.H6(id={'type': 'WPdynamic', 'index': "StringTime"},
                                    ...
                                    [
                                        html.Div(
                                                [html.H1(
                                                    ...
                                                    children=str(CWJSON[
                                                                     "BarometricPressureSeaLevel"]) + BUnits(),
                                                    style={
                                                        "font-size": maintextsize,
                                                        "color": maintextcolor}),
                                        ...
                                        html.Div(
                                                [html.H1(
                                                    ...
                                                    children=str(CWJSON[
                                                                     "WindDirection"]) + " deg " + calc_wind_quadrant(
                                                            CWJSON[
                                                                "WindDirection"]),
                                                    style={
                                                        "font-size": maintextsize,
                                    ...
                                    [
                                        ...
                                        html.Div(
                                                [html.H1(
                                                    ...
                                                    children=str(CWJSON[
                                                                     "CalendarDayRain"]) + RUnits(),
                                                    style={
                                                        "font-size": maintextsize,
                                                        "color": maintextcolor}),
                                                 html.P("Daily Rain", style={
                                                     "color": subtextcolor})
                                                 ], id="ot1",
                                                className="mini_container", ),
                                        html.Div(
