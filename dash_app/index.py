...
def updateWeatherImagePage(n_intervals, id, value):
    ...
    if ((n_intervals % (1 * 6)) == 0) or (
            n_intervals == 0):  # 1 minutes -10 second timer
...
def updateWeatherUpdate(n_intervals, id, value):
    if ((n_intervals % (1 * 6)) == 0) or (
            n_intervals == 0):  # 5 minutes -10 second timer
        ...
        if (id['index'] == "StringTime"):
            ...
            value = str(weather_page.CWJSON[id['index']]) + " " + \
                    weather_page.CWJSON[id['index'] + 'Units']
            ...
            value = str(wind_dir) + weather_page.CWJSON[
                id['index'] + 'Units'] + " " + wind_dir_str
            return [value]
        UpdateCWJSONLock.acquire()
        value = str(weather_page.CWJSON[id['index']]) + " " + \
                weather_page.CWJSON[id['index'] + 'Units']
...
def updateLightningUpdate(n_intervals, id, value):
    if (True):
        ...
        # if ((n_intervals % (5*6)) == 0) or (n_intervals ==0): # 5 minutes -10 second timer
        print("--->>>updateLightningUpdate", datetime.datetime.now(),
              n_intervals)
